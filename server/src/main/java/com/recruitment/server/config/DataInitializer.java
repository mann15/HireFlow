...
public class DataInitializer implements CommandLineRunner {
        ...
        public void run(String... args) throws Exception {
                // Always ensure required roles and default users exist
                Role superAdminRole = ensureRole("SUPER_ADMIN");
                Role adminRole = ensureRole("ADMIN");
                Role recruiterRole = ensureRole("RECRUITER");
                Role hrRole = ensureRole("HR");
                Role interviewerRole = ensureRole("INTERVIEWER");
                Role reviewerRole = ensureRole("REVIEWER");
                Role candidateRole = ensureRole("CANDIDATE");
                Role viewerRole = ensureRole("VIEWER");

                ensureUser("superadmin@hireflow.com", "Super@123", "Super", "Admin", superAdminRole);
                ensureUser("admin@hireflow.com", "Admin@123", "System", "Admin", adminRole);
                User recruiter = ensureUser("recruiter1@hireflow.com", "Recruiter@123", "Rita", "Recruiter",
                                recruiterRole);
                ensureUser("hr1@hireflow.com", "HR@123", "Hannah", "HR", hrRole);
                User interviewer = ensureUser("interviewer1@hireflow.com", "Interviewer@123", "Ilan", "Interviewer",
                                interviewerRole);
                ensureUser("reviewer1@hireflow.com", "Reviewer@123", "Raj", "Reviewer", reviewerRole);
                ensureUser("viewer1@hireflow.com", "Viewer@123", "Vicky", "Viewer", viewerRole);
                User candidateUser = ensureUser("candidate1@hireflow.com", "Candidate@123", "Cathy", "Candidate",
                                candidateRole);

                // Skip demo dataset if it already exists
                ...
                // Skills
                Skills javaSkill = skillsRepository
                                .save(Skills.builder().skillName("Java").category("Backend").build());
                Skills reactSkill = skillsRepository
                                .save(Skills.builder().skillName("React").category("Frontend").build());

                // Proficiency levels
                ProficiencyLevels junior = proficiencyLevelsRepository
                                .save(ProficiencyLevels.builder().levelName("Junior").description("Basic").rank(1)
                                                .build());
                ProficiencyLevels mid = proficiencyLevelsRepository
                                .save(ProficiencyLevels.builder().levelName("Mid").description("Intermediate").rank(2)
                                                .build());
                ProficiencyLevels senior = proficiencyLevelsRepository
                                .save(ProficiencyLevels.builder().levelName("Senior").description("Expert").rank(3)
                                                .build());
        ...
        private User ensureUser(String email, String rawPassword, String firstName, String lastName, Role role) {
